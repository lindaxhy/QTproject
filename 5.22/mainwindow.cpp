    ...
    , ui(new Ui::MainWindow)
{
    ...
    rightLayout->addWidget(ui->textEdit, 1);
    rightLayout->addWidget(ui->pushButton);
    rightLayout->addWidget(ui->btnHistoryEvent);
    rightLayout->addWidget(ui->btnExportStarMap);
    // 左侧主垂直布局
    QVBoxLayout *leftLayout = new QVBoxLayout();
    leftLayout->addWidget(calendar); // 添加日历控件
    // 创建四个标签的水平布局
    QHBoxLayout *labelsLayout = new QHBoxLayout();
    labelsLayout->addWidget(ui->labelTemp);      // 温度标签
    labelsLayout->addWidget(ui->labelWeather);   // 天气标签
    labelsLayout->addWidget(ui->labelHumidity);  // 湿度标签
    labelsLayout->addWidget(ui->labelWind);      // 风力标签
    // 将标签布局添加到左侧垂直布局
    leftLayout->addLayout(labelsLayout);
    // 主水平布局
    QHBoxLayout *mainLayout = new QHBoxLayout();
    mainLayout->addLayout(leftLayout);   // 左侧整体占拉伸空间
    mainLayout->addLayout(rightLayout);  // 右侧固定宽度
