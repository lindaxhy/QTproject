    ...
    , ui(new Ui::MainWindow)
{
    ...
    /*QSettings settings;
    QString savedFont = settings.value("Font", "Arial").toString();
    qDebug() << "Loaded font:" << savedFont; // 打印日志
    setApplicationFont(savedFont);*/
    QSettings settings;
    QString fontName = settings.value("Font").toString();
    if (!fontName.isEmpty()) {
        setApplicationFont(fontName);
    }
...
void MainWindow::setApplicationFont(const QString& fontName) {
   /* QFont font(fontName);
    // 设置全局字体（或仅设置当前窗口字体）
    ...
    update();
    qDebug() << "当前应用字体：" << QFontInfo(font).family();
}

// MainWindow.cpp
void MainWindow::setColorMode(bool isDarkMode) {
    if (isDarkMode) {
        // 深色模式样式
        this->setStyleSheet(
...
// MainWindow.cpp
/*void MainWindow::on_actionColorMode_triggered() {
    color_mode *colorWindow = new color_mode(this); // 传递 MainWindow 指针
    colorWindow->show();
}*/
void MainWindow::onOpenColorMode() {
    color_mode *colorWindow = new color_mode();
    connect(colorWindow, &color_mode::colorModeChanged, this, &MainWindow::setColorMode); // 连接信号与槽
    colorWindow->show();
}
